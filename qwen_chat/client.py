...
class Qwen:
    ...
    def _build_payload(
        ...
        for msg in messages:
            ...
            # Generate new fid for this message
            fid = str(uuid.uuid4())
            
            # Look for image blocks or files
            files_payload = []
            content_blocks = []
            ...
            for block in validated_msg.blocks:
                if block.block_type == "text":
                    ...
                    if block.url:
                        block_filename = getattr(block, "filename", None) or "image.jpg"
                        block_file_id = getattr(block, "file_id", None) or str(uuid.uuid4())
                        block_size = getattr(block, "size", None) or 0
                        img_file = {
                            "type": "image",
                            "file": {
                                ...
                                "meta": {
                                    "name": block_filename,
                                    "size": block_size,
                                    "content_type": block.image_mimetype or "image/jpeg",
                                },
                            ...
                            "file_type": block.image_mimetype or "image/jpeg",
                            "showType": "image",
                            "file_class": "vision",
                            "uploadTaskId": str(uuid.uuid4()),
                        }
                        files_payload.append(img_file)
                        content_blocks.append({"type": "image", "image": str(block.url)})
                    else:
                        content_blocks.append({"type": "image", "image": ""})
                else:
                    content_blocks.append({"type": block.block_type})

            # Check if simple text content is preferred
            msg_content = ""
            text_blocks = [b.text for b in validated_msg.blocks if b.block_type == "text"]
            if text_blocks:
                msg_content = "\n".join(text_blocks)
    ...
    def _process_response(self, response: requests.Response) -> ChatResponse:
        ...
        for event in client.events():
            if event.data:
                self.logger.debug(f"Raw SSE Event Data: {event.data}")
                try:
                    data = json.loads(event.data)
                    if "response.created" in data:
                        new_parent = data["response.created"].get("response_id")
                        if hasattr(self, "chat") and self.chat:
                            self.chat._parent_id = new_parent
                    if "choices" in data:
