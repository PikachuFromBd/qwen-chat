...
class Completion:
    ...
    def _get_or_create_chat_id(self, model="qwen-max-latest") -> str:
        if not self._chat_id:
            ...
            r = requests.post(url, headers=self._client._build_headers(), json=payload, timeout=30)
            r.raise_for_status()
    ...
    async def _aget_or_create_chat_id(self, model="qwen-max-latest") -> str:
        if not self._chat_id:
            import time
            url = self._client.base_url + EndpointAPI.new_chat
            payload = {
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=self._client._build_headers(), json=payload, timeout=30) as r:
                    r.raise_for_status()
    ...
    def create(
        ...
        # Validate and normalize messages to ChatMessage objects
        normalized_messages = []
        for msg in messages:
        ...
        # Auto-upload any local files in ImageBlocks
        for message in messages:
            if hasattr(message, "blocks") and message.blocks:
                for block in message.blocks:
                    if getattr(block, "block_type", None) == "image":
                        if not getattr(block, "url", None) and (getattr(block, "path", None) or getattr(block, "image", None)):
                            if block.path:
                                file_result = self.upload_file(file_path=str(block.path))
                            elif block.image:
                                base64_str = block.image.decode("utf-8")
                                file_result = self.upload_file(base64_data=base64_str)
                            
                            block.url = file_result.file_url
                            block.file_id = file_result.file_id
                            block.image_mimetype = file_result.image_mimetype
                            if block.path:
                                block.filename = os.path.basename(str(block.path))
                                block.size = os.path.getsize(str(block.path))
        ...
        chat_id = self._get_or_create_chat_id(model)
    ...
    async def acreate(
        ...
        # Auto-upload any local files in ImageBlocks asynchronously
        for message in messages:
            if hasattr(message, "blocks") and message.blocks:
                for block in message.blocks:
                    if getattr(block, "block_type", None) == "image":
                        if not getattr(block, "url", None) and (getattr(block, "path", None) or getattr(block, "image", None)):
                            if block.path:
                                file_result = await self.async_upload_file(file_path=str(block.path))
                            elif block.image:
                                base64_str = block.image.decode("utf-8")
                                file_result = await self.async_upload_file(base64_data=base64_str)
        ...
        try:
            if tools:
                ...
                chat_id = await self._aget_or_create_chat_id(model)
                payload = self._client._build_payload(
                    messages=messages,
                    model=model,
                    temperature=temperature,
